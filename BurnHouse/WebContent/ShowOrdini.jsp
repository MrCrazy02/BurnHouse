...
<% Iterator it=ordini.iterator();
	...
	<%=request.getSession().getAttribute("cognome") %></span><h3 class="number">Ordine n.<%=order.GetCode() %></h3>
	<span class="date">Data effettuazione:<br><%=order.GetDate() %></span></div>
	...
	<img src="${pageContext.request.contextPath}/Immagini/<%=prod.getImg()%>" alt="Immagine non disponibile" class="orderimg"></a>
	<h3 class="about"><a href ="Dettagli.jsp?id=<%=prod.getCode()%>" target="_blank" rel="noopener"><%=prod.getName()%></a><br>
	<%=prod.getCapacity() %>cl</h3>
	<div class="price">Prezzo
	<br>€<%=String.format("%.2f", prod.getPrice())%> </div>
	</div>
	<%} %>
